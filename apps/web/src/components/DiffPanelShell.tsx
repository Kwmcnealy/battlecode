  ...
  return (
    <div
      className={cn(
        ...
        props.mode === "inline"
          ? "w-[42vw] min-w-[360px] max-w-[560px] shrink-0 border-l border-primary/35 shadow-[var(--glow-panel)]"
          : "w-full",
      )}
    >
      {shouldUseDragRegion ? (
  ...
  return (
    <>
      <div className="relative min-w-0 flex-1">
        <Skeleton className="absolute left-0 top-1/2 size-6 -translate-y-1/2 border border-border/50" />
        <Skeleton className="absolute right-0 top-1/2 size-6 -translate-y-1/2 border border-border/50" />
        <div className="flex gap-1 overflow-hidden px-8 py-0.5">
          <Skeleton className="h-6 w-16 shrink-0" />
          <Skeleton className="h-6 w-24 shrink-0" />
          <Skeleton className="h-6 w-24 shrink-0 max-sm:hidden" />
        </div>
      </div>
...
export function DiffPanelLoadingState(props: { label: string }) {
  return (
    <div className="flex min-h-0 flex-1 flex-col p-2">
      <div
        className="flex min-h-0 flex-1 flex-col overflow-hidden border border-border/60 bg-card/35"
        role="status"
        aria-live="polite"
