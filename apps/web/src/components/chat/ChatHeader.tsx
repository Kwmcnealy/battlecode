...
import { DiffIcon, MessageSquareIcon, TerminalSquareIcon, WorkflowIcon } from "lucide-react";
...
const SYMPHONY_UNAVAILABLE_MESSAGE =
  "Symphony is unavailable until this thread has an active project.";
    ...
    <div className="@container/header-actions flex min-w-0 flex-1 items-center gap-2">
      <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden sm:gap-3">
        ...
        <ToggleGroup
          ...
          onValueChange={(value) => {
            const next = value[0];
            if (next === "chat" || next === "terminal" || next === "symphony") {
        ...
        >
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <ToggleGroupItem
                  aria-label="Show Symphony orchestration view"
                  className="h-7 min-w-7 rounded-md border border-transparent px-2.5 text-[11px] text-muted-foreground/85 shadow-none hover:bg-[color-mix(in_srgb,var(--theme-primary)_8%,var(--cg-card))] hover:text-foreground data-pressed:border-[color-mix(in_srgb,var(--theme-primary)_32%,var(--border))] data-pressed:bg-[color-mix(in_srgb,var(--theme-primary)_18%,var(--cg-card))] data-pressed:text-foreground data-pressed:shadow-[inset_0_1px_0_color-mix(in_srgb,var(--theme-primary)_20%,transparent),0_0_0_1px_color-mix(in_srgb,var(--theme-primary)_12%,transparent)] sm:min-w-[5.75rem]"
                  value="symphony"
                  disabled={!terminalAvailable}
                >
                  <WorkflowIcon className="size-3.5" />
                  <span className="hidden text-[11px] sm:inline">Symphony</span>
                </ToggleGroupItem>
              }
            />
            <TooltipPopup side="bottom">
              {terminalAvailable ? "Show Symphony orchestration" : SYMPHONY_UNAVAILABLE_MESSAGE}
            </TooltipPopup>
          </Tooltip>
        </ToggleGroup>
        <h2
